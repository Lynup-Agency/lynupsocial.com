...
else
{
    ...
    {
        $to = $send_to['val'];
        if (wp_mail($to, $subject, $message, $headers)){$sent ++;}
    }
...
function formcraft_captcha($value, $valid, $req, $min, $max, $tool, $con)
{
    ...
    if (isset($_SESSION["security_number_$id"]))
    {
        ...
        $exists = array_search( strtoupper($value), $_SESSION["security_number_$id"]);
...
function addcopy() {
echo '<a href="http://www.xum.ir">سایت آپلود عکس</a>';
}
...
function formcraft_add() {
    ...
    if ($_POST['type_form']=='duplicate')
    {
        ...
        if (array_search($dup, $restricted) && $_SERVER['HTTP_HOST'] == 'ncrafts.net')
        {
            $result2['Error'] = 'Cannot duplicate template form';
            echo json_encode($result2);
            die();
        }
...
// $data = file_get_contents(plugins_url('formcraft/file-upload/server/php/files/'.$_POST['import_form']));

        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, plugins_url('formcraft/file-upload/server/php/files/'.$_POST['import_form']));
        ...
        if (empty($data))
        {
            ...
            echo json_encode($result2);
            unlink(__DIR__.'/file-upload/server/php/files/'.$_POST['import_form']);
            die();
        }
...
// Main Form CSS
    wp_enqueue_style( 'main_style_fc', plugins_url( 'css/nform_style.css', __FILE__ ));
...
// Datepicker
    wp_enqueue_script( 'js_lib', plugins_url( 'libraries/js_libraries.js', __FILE__ ), array('jquery','jquery-ui-core','jquery-ui-mouse', 'jquery-ui-widget', 'jquery-ui-sortable', 'jquery-ui-slider'));
...
function widget($args, $instance)
{
    ...
    if (!(empty($id)))
    {
        ...
        <div id="myModal'.$id.'" class="modal hide fc-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none">
...
function formcraft_menu() {
    ...
    {
        $page = add_menu_page( 'FormCraft - Form Builder', 'FormCraft', 'read', 'survey_builder', 'formcraft_menu_options', plugins_url('formcraft/images/icon.png' ),'31.21' );
    }
    else
    {
